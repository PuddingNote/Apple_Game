...
public class OptionManager : MonoBehaviour
{
    ...
    // 현재 씬(활성 씬)에 속한 Canvas만 찾는 함수.
    // FindObjectOfType<Canvas>()는 GameBootstrap이 DontDestroyOnLoad로 만드는
    // 강제 업데이트 차단 창의 Canvas까지 걸릴 수 있어 씬 소속으로 한정한다.
    ...
    {
        // 강제 업데이트 차단 창이 떠 있으면 Pause 패널이 뒤로가기로 뜨지 않게 막는다
    ...
    {
        // 대기 중인 전면광고가 있으면 씬 전환 전에 표시(3판마다 1회), 없으면 바로 전환
